...
def evaluate_thresholds(model, features, labels):
    """Evaluate different thresholds and find the one with the highest F1 score."""
    probabilities = [model.prob_classify(features).prob('True') for features in features]
    ...
    for threshold in thresholds:
        predictions = ['True' if prob >= threshold else 'False' for prob in probabilities]
...
def classify_with_threshold(model, features, threshold):
    """Classify data using the model and a specified threshold."""
    probabilities = [model.prob_classify(features).prob('True') for features in features]
    predictions = ['True' if prob >= threshold else 'False' for prob in probabilities]
    return predictions
...
if __name__ == "__main__":
    ...
    train_features = [features for features, _ in train_features_labels]
    train_labels = [label for _, label in train_features_labels]
    ...
    best_threshold, best_f1 = evaluate_thresholds(maxent_model, train_features, train_labels)
    print(f"Best threshold: {best_threshold}, Best F1 score: {best_f1}")

    # Classify the development set using the best threshold
    print("Evaluating on dev set with the best threshold...")
    dev_features = [features for features, _ in dev_features_labels]
    dev_labels = [label for _, label in dev_features_labels]
    dev_predictions = classify_with_threshold(maxent_model, dev_features, best_threshold)
    ...
    annotated_data = [(features, pred) for features, pred in zip(dev_features, dev_predictions)]
